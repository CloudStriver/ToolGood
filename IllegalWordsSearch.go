...
import (
	...
	"errors"
	"fmt"
	"github.com/CloudStriver/ToolGood/tool"
...
func (this *IllegalWordsSearch) toSenseWord(c int32) int32 {
	...
	if this.UseSimplifiedChineseConverter {
		if c >= 0x4e00 && c <= 0x9fa5 {
			return tool.GetSimplified(int(c - 0x4e00))
		}
	}
	return c
}
...
func (this *IllegalWordsSearch) LoadFromDB(config *MongoConfig) {
	clientOptions := options.Client().ApplyURI(config.DataSource)
	...
	this.Mongo = client.Database(config.DB)
	if err = this.CreateCollection(); err != nil {
		panic(err)
	}
	this.LoadData()
...
func (this *IllegalWordsSearch) CreateCollection() error {
	_, err := this.Mongo.Collection(KeywordsCollection).Indexes().CreateOne(context.Background(),
		mongo.IndexModel{
			Keys:    bson.D{{KeywordBson, 1}},
...
func (this *IllegalWordsSearch) LoadData() {
	keywords, err := LoadField[Keywords](this.Mongo, KeywordsCollection, KeywordBson)
	if err != nil {
		panic(err)
	}
	this.SetKeywords(lo.Map[Keywords, string](keywords, func(item Keywords, index int) string {
		return item.Keyword
	}))
}
...
func LoadField[T any](Mongo *mongo.Database, collectionName string, typeJson string) ([]T, error) {
	...
	cur, err := Mongo.Collection(collectionName).Find(ctx,
		bson.D{})
	if err != nil {
		return nil, err
	}
